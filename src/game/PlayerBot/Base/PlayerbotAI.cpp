...
void PlayerbotAI::GetCombatTarget(Unit* forcedTarget)
{
    ...
    if (deep_forcedTarget_guid)
    {
        ...
        if (m_mgr.m_confDebugWhisper)
            TellMaster("Changing target to %s by force!", m_bot->GetMap()->GetUnit(deep_forcedTarget_guid)->GetName());
        m_targetCombat = m_bot->GetMap()->GetUnit(deep_forcedTarget_guid);
    ...
    if (!deep_m_targetCombat_guid ||
        m_bot->GetMap()->GetUnit(deep_m_targetCombat_guid)->IsDead() ||
        !m_bot->GetMap()->GetUnit(deep_m_targetCombat_guid)->IsInWorld() ||
    ...
    if (!deep_m_targetCombat_guid && (m_combatOrder & ORDERS_ASSIST) && m_targetAssist)
    {
    ...
    if (!m_bot->GetMap()->GetUnit(deep_m_targetCombat_guid))
    {
        m_targetType = TARGET_NORMAL;
        m_targetChanged = false;
        deep_m_targetCombat_guid.Clear();
    ...
    if (m_bot->GetMap()->GetUnit(deep_m_targetCombat_guid)->GetTypeId() == TYPEID_PLAYER &&
    ...
    m_bot->SetSelectionGuid((deep_m_targetCombat_guid));
    SetIgnoreUpdateTime(1);
