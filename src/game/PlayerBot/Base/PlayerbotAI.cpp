...
void PlayerbotAI::GetCombatTarget(Unit* forcedTarget)
{
    ...
    Unit* deep_m_targetCombat = new Unit(*m_targetCombat);
    if(forcedTarget)
        Unit* deep_forcedTarget = new Unit(*forcedTarget);
    ...
    if (!deep_forcedTarget && (m_combatOrder & ORDERS_PROTECT) && m_targetProtect)
    {
        ...
        {
            deep_forcedTarget = candidateTarget;
            m_targetType = TARGET_THREATEN;
            if (m_mgr.m_confDebugWhisper)
                TellMaster("Changing target to %s to protect %s", deep_forcedTarget->GetName(), m_targetProtect->GetName());
        }
    }

    // are we forced on a target?
    if (deep_forcedTarget)
    {
        // forced to change target to current target == null operation
        if (deep_forcedTarget && deep_forcedTarget == deep_m_targetCombat)
            deep_m_targetCombat = nullptr;
            deep_forcedTarget = nullptr;
            return;

        if (m_mgr.m_confDebugWhisper)
            TellMaster("Changing target to %s by force!", deep_forcedTarget->GetName());
        m_targetCombat = deep_forcedTarget;
    ...
    if (!deep_m_targetCombat ||
        deep_m_targetCombat->IsDead() ||
        !deep_m_targetCombat->IsInWorld() ||
        !m_bot->CanAttack(deep_m_targetCombat) ||
        !m_bot->IsInMap(deep_m_targetCombat))
    {
        deep_m_targetCombat = nullptr;
        m_targetCombat = nullptr;
    }
    ...
    if (deep_m_targetCombat)
    {
        ...
        if ((IsNeutralized(deep_m_targetCombat) && !m_ignoreNeutralizeEffect))
        {
            deep_m_targetCombat = nullptr;
            deep_forcedTarget = nullptr;
            m_targetCombat = nullptr;
    ...
    if (!deep_m_targetCombat && (m_combatOrder & ORDERS_ASSIST) && m_targetAssist)
    {
        ...
        {
            deep_m_targetCombat = candidateTarget;
            if (m_mgr.m_confDebugWhisper)
    ...
    if (deep_m_targetCombat->GetTypeId() == TYPEID_PLAYER && dynamic_cast<Player*>(deep_m_targetCombat)->duel)
    {
    ...
    m_bot->SetSelectionGuid((deep_m_targetCombat->GetObjectGuid()));
    SetIgnoreUpdateTime(1);
